...
def get_cmd():
    ...
    # for inference with saved weight
    parser.add_argument("--model_weight_path", default='', type=str, help="saved model weight path")
...
def main():
    ...
    try:
        model.load_state_dict(
            torch.load(
                conf['model_weight_path'],
                map_location=device,
                weights_only=True
            )
        )
        print(f"loaded model weights from {conf['model_weight_path']}")
    except:
