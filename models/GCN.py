...
class AGCN(nn.Module):
    ...
    def __init__(
        self,input_dim,output_dim, layer=3, dropout=0.2, bias=False
    ):
    ...
    def weight_cosine_matrix_div(self,emb):
        ...
        # emb: shape (n, d)
        emb = torch.matmul(emb,self.cos_weight) # shape (n, d)
        node_norm = F.normalize(emb,p=2,dim=-1)
        cos_adj = torch.mm(node_norm, node_norm.transpose(-1, -2)) # shape (n, n)
        return cos_adj
